export default function move(gameState) {
  ...
  function countReachableSpaces(position, visited, allSnakeBodies, depth, maxDepth) {
    ...
    for (let h = 0; h < hazards.length; h++) {
      if (position.x === hazards[h].x && position.y === hazards[h].y) {
        return 0;
      }
    }
    ...
    for (let m = 0; m < futureMoves.length; m++) {
      ...
      for (let h = 0; h < hazards.length; h++) {
        if (move.x === hazards[h].x && move.y === hazards[h].y) {
          isOccupied = true;
          break;
        }
      }
  ...
  function findAttackTarget() {
    ...
    let minDistance = manhattanDistance(myHeadPosition, potentialTargets[0].body[0]);

    for (let i = 1; i < potentialTargets.length; i++) {
      const distance = manhattanDistance(myHeadPosition, potentialTargets[i].body[0]);
      if (distance < minDistance) {
        minDistance = distance;
        closestTarget = potentialTargets[i];
      }
    }
    return closestTarget.body[0];
  }
  ...
  if (finalMove) {
    ...
    for (let h = 0; h < hazards.length; h++) {
      if (nextPos.x === hazards[h].x && nextPos.y === hazards[h].y) {
